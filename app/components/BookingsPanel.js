...
export default function BookingsPanel({ bookings, cancelled, onClose, onCancel, onSessionCancel, onReschedule }) {
  ...
  return <section className="account-panel">
    ...
    {list.length ? <div className="booking-list">{list.map((item) => <BookingCard key={item.reference} item={item} cancelled={cancelled} onCancel={onCancel} onSessionCancel={onSessionCancel} onReschedule={onReschedule} />)}</div> : <div className="empty-state">No {cancelled ? 'cancelled' : 'current'} bookings yet.</div>}
  </section>;
}

function BookingCard({ item, cancelled, onCancel, onSessionCancel, onReschedule }) {
  ...
  const activeSession = sessions.find((session) => activeStatuses.includes(session.status));

  const confirmDelete = () => {
    if (text !== 'DELETE') return;
    if (confirm === 'booking') onCancel(item.reference);
    if (confirm === 'session') onSessionCancel(item.reference, activeSession.number);
    setConfirm(null);
    setText('');
  };
  ...
  return <article className="booking-card">
    <div>
      ...
      <div className="session-list">
        ...
        {sessions.map((session) => {
          const isActive = activeSession?.number === session.number;
          return <div className={`session-row ${isActive ? 'active-session' : ''}`} key={session.number}>
            <span>Session {session.number}: {session.date} at {session.time}</span>
            {!cancelled && isActive && <span className="session-actions"><button type="button" className="action-button reschedule-button" onClick={() => setEditing(session)}>Reschedule</button><button type="button" className="action-button cancel-button" onClick={() => { setConfirm('session'); setText(''); }}>Cancel</button></span>}
          </div>;
        })}
      </div>
    ...
    {confirm && <div className="confirm-box"><b>Type DELETE to confirm</b><input value={text} onChange={(event) => setText(event.target.value)} placeholder="DELETE" autoFocus /><button type="button" className="action-button cancel-button" disabled={text !== 'DELETE'} onClick={confirmDelete}>Confirm delete</button><button type="button" className="action-button neutral-button" onClick={() => { setConfirm(null); setText(''); }}>Keep it</button></div>}
  </article>;
}
